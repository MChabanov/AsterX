...
class lintp_ND_t {
 ...
 private:
  ...
  inline CCTK_DEVICE CCTK_HOST CCTK_ATTRIBUTE_ALWAYS_INLINE size_t find_index(
  ...
  struct compressed_indexr_t {
    template <size_t N = 0>
    static inline CCTK_DEVICE CCTK_HOST CCTK_ATTRIBUTE_ALWAYS_INLINE size_t value(
  ...
  static inline CCTK_DEVICE CCTK_HOST CCTK_ATTRIBUTE_ALWAYS_INLINE T const linterp1D(
  ...
  struct recursive_interpolate_single {
    static inline CCTK_DEVICE CCTK_HOST CCTK_ATTRIBUTE_ALWAYS_INLINE T const linterp(
 ...
 public:
  inline CCTK_INLINE CCTK_DEVICE CCTK_HOST T const &operator[](const int i) const { return y[i]; }

  inline CCTK_INLINE CCTK_DEVICE CCTK_HOST int size(size_t const which_dim) { return num_points[which_dim]; }

  inline CCTK_INLINE CCTK_DEVICE CCTK_HOST T const &xmin(size_t which_dim) const { return x[which_dim][0]; };

  template <size_t which_dim>
  inline CCTK_INLINE CCTK_DEVICE CCTK_HOST T const &xmin() const {
    return x[which_dim][0];
  };

  inline CCTK_INLINE CCTK_DEVICE CCTK_HOST T const &xmax (size_t which_dim) const {
    return x[which_dim][num_points[which_dim] - 1];
  };

  template <size_t which_dim>
  inline CCTK_INLINE CCTK_DEVICE CCTK_HOST T const &xmax() const {
    return x[which_dim][num_points[which_dim] - 1];
  };

  template <size_t... vars, typename... Xt>
  inline CCTK_INLINE CCTK_DEVICE CCTK_HOST vec_t<sizeof...(vars)> interpolate(Xt const &... xin) const {
    static_assert(
  ...
  CCTK_INLINE CCTK_DEVICE CCTK_HOST lintp_ND_t() = default;

  template <typename Yt, typename... Xt>
  CCTK_INLINE CCTK_DEVICE CCTK_HOST lintp_ND_t(Yt &&__y, std::array<size_t, dim> &&__num_points, Xt &&... __x)
